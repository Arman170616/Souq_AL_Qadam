...
export default function VendorOrdersPage() {
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: () => { toast.success(t('ven.orders.updated')); qc.invalidateQueries({ queryKey: ['vendor-orders'] }); },
    onError: () => toast.error(t('ven.orders.failedUpdate')),
  });

  return (
    <motion.div initial={{opacity:0}} animate={{opacity:1}} className="space-y-5">
      <h2 className="text-2xl font-black text-white">{t('ven.orders.title')}</h2>

      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 max-w-xs">
          <Search size={14} className="absolute inset-s-3 top-1/2 -translate-y-1/2 text-white/40"/>
          <input value={search} onChange={e => setSearch(e.target.value)}
            placeholder={t('ven.orders.search')} className="glass-input ps-8 text-sm py-2"/>
        </div>
        <div className="flex gap-1 glass-dark rounded-xl p-1 flex-wrap">
          {STATUSES.map(s => (
            <button key={s} onClick={() => setFilter(s)}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold capitalize transition-all ${filter === s ? 'bg-indigo-500 text-white' : 'text-white/50 hover:text-white'}`}>
              {s}
            </button>
          ))}
        </div>
      ...
      {isLoading ? (
        ...
        <div className="glass-card p-10 text-center text-white/40">{t('ven.orders.noOrders')}</div>
      ) : (
            ...
            <div key={o.order_number} className="glass-card overflow-hidden">
              <div className="flex items-center justify-between p-4 cursor-pointer hover:bg-white/3 transition-colors"
                ...
                <div className="flex items-center gap-4">
                  ...
                  {NEXT_STATUS[o.status] && (
                    <button
                      onClick={e => { e.stopPropagation(); statusMutation.mutate({ orderNumber: o.order_number, status: NEXT_STATUS[o.status] }); }}
              ...
              {expanded === o.order_number && o.shipping_address && (
                <motion.div initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}}
                  ...
                  <p className="text-xs text-white/50 font-semibold uppercase tracking-wider mb-2">{t('ven.orders.shippingAddress')}</p>
