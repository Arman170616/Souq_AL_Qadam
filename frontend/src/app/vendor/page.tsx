...
export default function VendorDashboard() {
  ...
  const STATS = [
    { label:'Total Revenue',   value: formatPrice(totalRevenue),  icon:DollarSign,  color:'from-indigo-500/20 to-purple-500/20', iconColor:'text-indigo-400' },
    { label:'Total Orders',    value: String(orders.length),      icon:ShoppingBag, color:'from-pink-500/20 to-rose-500/20',    iconColor:'text-pink-400' },
    { label:'Products',        value: String(products.length),    icon:Package,     color:'from-amber-500/20 to-orange-500/20', iconColor:'text-amber-400' },
    { label:'Avg Rating',      value: vendorMe ? `${Number(vendorMe.rating).toFixed(1)} ★` : '—', icon:Star, color:'from-green-500/20 to-emerald-500/20', iconColor:'text-green-400' },
  ];

  return (
    <motion.div initial="hidden" animate="show" variants={stagger} className="space-y-6 p-6 lg:p-8">

      {/* No vendor profile warning — only show for non-admin vendors */}
      {noVendorProfile && user?.role !== 'admin' && (
        <motion.div variants={fadeUp} className="glass-card p-5 border border-amber-500/30 bg-amber-500/10">
          <p className="font-bold text-amber-400 text-sm mb-1">Vendor profile not found</p>
          <p className="text-white/60 text-sm mb-3">You haven&apos;t set up your vendor profile yet. Create one to start selling.</p>
          <Link href="/vendor/register" className="btn-primary text-xs px-4 py-2 rounded-lg inline-flex items-center gap-1.5">
            <ArrowRight size={13}/> Create Vendor Profile
          </Link>
        </motion.div>
      )}
      ...
      <motion.div variants={fadeUp} className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-white">
            {vendorMe ? vendorMe.shop_name : 'Vendor Dashboard'}
          </h1>
          <p className="text-white/50 text-sm mt-0.5">
            {vendorMe ? `Status: ${vendorMe.status}` : noVendorProfile ? 'Profile not set up yet' : '—'}
          </p>
        </div>
        <AddProductDropdown />
      </motion.div>
      ...
      <motion.div variants={fadeUp} className="glass-card p-6 bg-linear-to-br from-emerald-500/10 to-teal-500/10 border border-emerald-500/20">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-bold text-white flex items-center gap-2">
            <TrendingUp size={16} className="text-emerald-400"/> Commission Balance
          </h3>
          <span className="text-xs text-white/40 bg-white/5 px-2 py-1 rounded-full">Rate: {commissionRate}%</span>
        </div>
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="text-center p-4 rounded-xl bg-amber-500/10 border border-amber-500/20">
            <p className="text-xs text-amber-400 uppercase tracking-wider mb-1">Pending</p>
            <p className="text-xl font-black text-amber-300">{formatPrice(pendingCommission)}</p>
            <p className="text-xs text-white/30 mt-0.5">awaiting settlement</p>
          </div>
          <div className="text-center p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
            <p className="text-xs text-emerald-400 uppercase tracking-wider mb-1">Settled</p>
        ...
        {commissionData?.commissions?.length > 0 && (
          <div className="mt-4 space-y-2">
            <p className="text-xs text-white/40 font-semibold uppercase tracking-wider">Recent commissions</p>
            {commissionData.commissions.slice(0, 3).map((c: { id: number; order_number: string; amount: string; rate: string; status: string; created_at: string }) => (
              <div key={c.id} className="flex items-center justify-between py-2 border-b border-white/5 last:border-0">
                <div>
                  ...
                  <span className="text-xs text-white/30 ml-2">{new Date(c.created_at).toLocaleDateString()}</span>
                </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <motion.div variants={fadeUp} className="glass-card p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-bold text-white flex items-center gap-2"><ShoppingBag size={16} className="text-indigo-400"/> Recent Orders</h3>
            <Link href="/vendor/orders" className="text-xs text-indigo-400 hover:text-indigo-300">View all →</Link>
          </div>
              ...
              {recentOrders.map((o: { order_number: string; total: string; status: string; created_at: string; item_count: number }) => (
                <div key={o.order_number} className="flex items-center justify-between p-3 rounded-xl bg-white/5">
                  <div>
                    ...
                    <p className="text-xs text-white/40">{o.item_count} item{o.item_count !== 1 ? 's' : ''} · {new Date(o.created_at).toLocaleDateString()}</p>
                  </div>
                  <div className="flex items-center gap-2">
        ...
        <motion.div variants={fadeUp} className="glass-card p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-bold text-white flex items-center gap-2"><Package size={16} className="text-amber-400"/> Your Products</h3>
            <Link href="/vendor/products" className="text-xs text-indigo-400 hover:text-indigo-300">Manage →</Link>
          </div>
          {products.length === 0 ? (
            <div className="text-center py-6">
              <p className="text-white/40 text-sm mb-3">No products yet</p>
              <Link href="/vendor/products/new" className="btn-primary text-xs px-4 py-2 rounded-lg">Add your first product</Link>
            </div>
          ) : (
            <div className="space-y-3">
